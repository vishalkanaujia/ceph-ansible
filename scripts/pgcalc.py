...
def populate(args):
	...
	if(minValue>calcValue):
		if args[7] == "replicated":
			print "sudo ceph osd pool create {} {} {}_ruleset_{};sudo ceph osd pool set {} size {};sudo ceph osd pool set {} min_size {}; while [ $(sudo ceph -s | grep creating -c) -gt 0 ]; do echo -n .;sleep 1; done".format(args[5],int(minValue),args[7],args[6],args[5],args[2],args[5],args[8])
		...
		if args[7] == "replicated":
			print "sudo ceph osd pool create {} {} {}_ruleset_{};sudo ceph osd pool set {} size {};sudo ceph osd pool set {} min_size {}; while [ $(sudo ceph -s | grep creating -c) -gt 0 ]; do echo -n .;sleep 1; done".format(args[5],int(calcValue),args[7],args[6],args[5],args[2],args[5],args[8])
		if args[7] == "erasurecoded":
